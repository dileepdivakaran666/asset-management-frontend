...
const GRNForm = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const url = id ? `/grns/${id}` : data.status === 'draft' ? '/grns?status=draft' : '/grns';
    ...
    <Box p={3}>
      ...
      <FormProvider {...methods}>
        <form noValidate>
          ...
          <HeaderForm register={register} control={control} errors={errors} setValue={setValue} />
          <LineItemTable
            control={control}
            fields={fields}
            append={append}
      ...
      <Dialog open={confirmCancelOpen} onClose={() => setConfirmCancelOpen(false)}>
        ...
        <DialogContent>You have unsaved changes. Are you sure you want to cancel?</DialogContent>
        <DialogActions>
