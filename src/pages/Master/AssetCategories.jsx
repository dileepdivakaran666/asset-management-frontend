      ...
      try {
        ...
        for (let i = 0; i < json.length; i++) {
          ...
          if (!['Active', 'Inactive'].includes(row.Status)) {
            throw new Error(
              `Invalid status "${row.Status}" in row ${i + 2}. Use "Active" or "Inactive"`
            );
          }
        }
          ...
          <TableBody>
            {categories.map((cat) => (
              <TableRow key={cat.id}>
                ...
                <TableCell>
                  <Button onClick={() => navigate(`/asset-categories/edit/${cat._id}`)}>
                    Edit
                  </Button>
                  <Button color="error" onClick={() => confirmDelete(cat._id)}>
                    Delete
                  </Button>
                </TableCell>
              </TableRow>
